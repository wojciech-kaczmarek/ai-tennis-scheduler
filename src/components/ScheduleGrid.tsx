...
function formatMatchPlayers(
  players: { name: string | null; placeholder_name: string; team: number | null }[],
 ...
 */
export function ScheduleGrid({ matches, editable, maxCourts, onMatchUpdate }: ScheduleGridProps) {
  ...
  const tournamentType: TournamentType =
    matches.length > 0 && matches[0].players.some((p) => p.team !== null) ? "doubles" : "singles";
  ...
  return (
    <div className="space-y-4">
              ...
              .map((match) => (
                <div key={match.id} className="flex items-center gap-4 p-3 bg-muted rounded">
                  {editable ? (
                    <>
                      ...
                      <div className="w-32">
                        <Select
                          ...
                          onValueChange={(value) => onMatchUpdate!(match.id, "court", parseInt(value, 10))}
                        >
                          <SelectTrigger>
                    ...
                    <span className="text-sm font-medium w-24">Match {match.match_order_on_court}</span>
                  )}

                  {/* Player names */}
                  <div className="flex-1 text-sm">{formatMatchPlayers(match.players, tournamentType)}</div>
                </div>
              ))}
          </div>
        </div>
      ...
      {courtNumbers.length === 0 && <div className="text-center py-8 text-muted-foreground">No matches scheduled</div>}
    </div>
  );
}
